...
isPalindrome'' [] = True
isPalindrome'' [x] = True
isPalindrome'' xs = ( head xs == meuLast xs ) && ( isPalindrome'' (init (tail xs)) )
{-
...
-}
compact [] = []
compact (x:xs) = (filter (==x) (x:xs)) ++ compact (removeList x (x:xs))
...
-}
encode [] = []
encode (x:xs) = compress ( [(a, b)| a <- ( filter (==x) (x:xs) ), b <- [length( filter (==x) (x:xs) )] ] ++ encode (removeList x xs) )
...
insertAt' el 1 xs = el:xs -- [el]++x:xs   --- [el]++[x]++xs
insertAt' el pos (x:xs) = x:(insertAt' el (pos-1) xs) --- [x]++(insertAt el (pos-1) xs)
{-
...
myAppend' xs (y:ys) = myAppend' (foldr (:) [y] xs) ys